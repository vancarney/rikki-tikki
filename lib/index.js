...
var AdapterManager, ClientLoader, CollectionManager, CollectionMonitor, EventEmitter, Model, RikkiTikkiAPI, SchemaManager, SchemaTree, SchemaTreeManager, SyncService, config, e, fs, p, path, _, _collections, _connections, _router, _types,
...
if (!fs.existsSync(p = config.get('data_path'))) {
  fs.mkdirSync(p);
}

if (!fs.existsSync(p = config.get('trees_path'))) {
  fs.mkdirSync(p);
}
