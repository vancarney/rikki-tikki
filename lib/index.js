...
var AdapterManager, ClientLoader, CollectionManager, CollectionMonitor, EventEmitter, Model, RikkiTikkiAPI, SchemaManager, SchemaTree, SchemaTreeManager, SyncService, cnf, e, fs, p, path, _, _collections, _connections, _router, _types,
...
if (!fs.existsSync(p = cnf.get('data_path'))) {
  fs.mkdirSync(p);
}

if (!fs.existsSync(p = cnf.get('trees_path'))) {
  fs.mkdirSync(p);
}
