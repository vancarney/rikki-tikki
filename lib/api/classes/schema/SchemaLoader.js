...
SchemaLoader = (function(_super) {
  ...
  SchemaLoader.prototype.saveAll = function() {
    return _.each(this.__schema, (function(_this) {
      return function(schema, name) {
        return _this.saveSchema(schema, name);
      };
    })(this));
  };

  SchemaLoader.prototype.saveSchema = function(name) {
    var schema;
    if ((schema = this.getSchema(name)) != null) {
      return fs.writeFile("" + RikkiTikkiAPI.SCHEMA_PATH + "/" + name + ".js", schema, function(e) {
        if (e) {
          return console.error("Failed to save schema '" + name + "' to file " + "'\nError: " + e);
        }
      });
    }
  };

  SchemaLoader.prototype.addSchema = function(name, schema) {
    if (!schema || !Util.isOfType(schema, RikkiTikkiAPI.Schema)) {
      throw 'schema must be an instance of RikkiTikkiAPI.Schema';
    }
    return this.__schema[key] = schema;
  };
  ...
  SchemaLoader.prototype.toJSON = function(readable) {
    return JSON.parse(this.toString(readable));
  };

  SchemaLoader.prototype.toString = function(readable) {
    return JSON.stringify({
      __meta__: this.__meta,
      __schemas__: this.__schema
