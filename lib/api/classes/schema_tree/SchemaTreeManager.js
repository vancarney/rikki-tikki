...
var RikkiTikkiAPI, SchemaTree, SchemaTreeManager, fs, path, _,
...
SchemaTreeManager = (function(_super) {
  ...
  function SchemaTreeManager() {
    this.__trees = {};
    this.__path = "" + (RikkiTikkiAPI.getOptions().config_path) + "/schema_trees";
    (this.load = (function(_this) {
      return function() {
        var e, file, stat, _i, _len, _ref, _results;
        try {
          stat = fs.statSync(_this.__path);
          console.log("" + _this.__path);
        } catch (_error) {
          e = _error;
          throw new Error(e);
          return false;
  ...
  SchemaTreeManager.prototype.createTree = function(name, data, callback) {
    ...
    if (!this.__trees[name]) {
      return (this.__trees[name] = new SchemaTree).create("" + this.__path + path.sep + name + ".json", JSON.stringify(data), callback);
    } else {
...
SchemaTreeManager.getInstance = function() {
  return this.__instance != null ? this.__instance : this.__instance = new SchemaTreeManager();
};
