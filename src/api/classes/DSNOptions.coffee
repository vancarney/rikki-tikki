...
class RikkiTikkiAPI.DSNOptions extends Object
  constructor:(options)->
    @setOptions options if options
  setOptions:(options)->
    throw new Error 'options was undefined' if !options
    options = RikkiTikkiAPI.Util.queryToObject options if RikkiTikkiAPI.Util.isOfType options, String
  ...
  __validate:(object)->
    ...
    _.each object, (value, key)=>
      ...
      if (RikkiTikkiAPI.Util.isOfType opts[key].type, Array) and opts[key].type.length
        found = false
        _.each opts[key].type, (v)=> 
          found = true if RikkiTikkiAPI.Util.isOfType value, v
  ...
  toJSON:->
    @__options
  toString:->
    RikkiTikki.Util.objectToQuery @__options