  ...
  refresh:(callback)->
    ...
    RikkiTikkiAPI.getSchemaManager().listSchemas (e, names)=>
      list = _.compact _.map names, (v)=>
        if !fs.existsSync _path = "#{RikkiTikkiAPI.getOptions().schema_path}#{path.sep}#{v}.js"
          @__collection.removeItemAt @getNames().indexOf v 
          return null 
        {name:v, updated:stats.mtime} if (!@filter v) and (stats = fs.statSync _path)?
      if list.length > 0
        _.each list, (value)=>
      ...
      callback? e, list
  startPolling:->
    @__iVal ?= fs.watch "#{RikkiTikkiAPI.getOptions().schema_path}", (event, filename) =>
      try
